%{
Exercise 2: Median filter computation
    Input arguments:
...
function out_img = Compute_Median(img, kernel_size)
    % Size of the picture
    [i_r,i_c] = size(img);
    % Size of the kernel
    k_r = kernel_size(1);
    ...
    buf_img = padarray(img, pad_size, 'replicate', 'both');
