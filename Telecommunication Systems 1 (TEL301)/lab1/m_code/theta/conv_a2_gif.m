...
X = zeros(1,length(t)); x1 = -T/2-10; x2 = T/2-10; % Input signal and its range
H = zeros(1,length(t)); h1 = -T/2+10; h2 = T/2+10; % Impulse responce and its range
Y = zeros(1,length(t)); xh1 = x1+h1; xh2 = x2+h2; % Convolution output

% h = zeros(1,length(t));
% h (t>-T/2+10 & t<T/2+10) = 1./sqrt(T);
h = rectangularPulse(h1,h2, h1:0.1:h2).*1/sqrt(T); h(t==0)=1/sqrt(T);
