...
figure('Name','Starting data')
%plot the samples  
plot(X(:, 1),X(:, 2),'b.')
title('Non normilised 2D data')
axis([0.5 6.5 2 8]); axis square;
fprintf('Program paused. Press enter to continue.\n\n');
fprintf('----------------------------------------\n\n');
pause;

% Normalize X by Standardization
[X_norm, mu, sigma] = featureNormalize(X);

% figure('Name','Standardize starting data')
% %plot the samples  
% plot(X_norm,'b.')
% title('Standardized 2D data')
...
for i = 1:length(eigenval)
    PCvariance(i) = eigenval(i) / sum(eigenval);
end
PCvariance = PCvariance';
